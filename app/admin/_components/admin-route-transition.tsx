...
type AdminTransitionClick = React.MouseEvent<HTMLAnchorElement>;
...
export function useAdminRouteNavigation(currentPath: string) {
  ...
  const navigateWithTransition = useCallback(
    async (event: AdminTransitionClick, targetPath: string) => {
      ...
      const exitRun = animateOut(getAdminRouteRoot(), shouldReduceMotion);
    ...
    [currentPath, router, shouldReduceMotion, warmRoute],
  );
...
export function AdminRouteTransition({ children, routeKey }: AdminRouteTransitionProps) {
  ...
  return (
    <div ref={rootRef} data-admin-route-root className="h-full min-h-full">
      {children}
    </div>
  );
}
...
function getAdminRouteRoot() {
  return document.querySelector<HTMLElement>('[data-admin-route-root]');
}
...
function getBubbleElements(root: HTMLElement) {
  const explicitBubbles = Array.from(
    root.querySelectorAll<HTMLElement>('[data-admin-bubble]:not([data-admin-bubble="false"])'),
  );
...
function collectAutoBubbles(root: HTMLElement) {
  ...
  const add = (element: Element | null) => {
    if (!isHTMLElement(element) || element.dataset.adminBubble === 'false') {
      return;
    }
...
function collectLayoutBubbles(parent: HTMLElement, depth = 0): HTMLElement[] {
  ...
  return Array.from(parent.children).flatMap((child) => {
    if (!isHTMLElement(child) || child.dataset.adminBubble === 'false') {
      return [];
    }
